...
def _window_label_from_seconds(seconds: Optional[int]) -> str:
    ...
    Matches the Codex CLI's get_limits_duration() logic:
    - <= ~1 day  -> "{hours}h-limit"
    - <= ~1 week -> "weekly-limit"
    - <= ~1 month -> "monthly-limit"
    - Otherwise   -> "annual-limit"
    """
    ...
    if minutes <= (24 * 60 + rounding_bias):
        hours = max(1, (minutes + rounding_bias) // 60)
        return f"{hours}h-limit"
    elif minutes <= (7 * 24 * 60 + rounding_bias):
        return "weekly-limit"
    elif minutes <= (30 * 24 * 60 + rounding_bias):
...
def _is_credits_bypass_disabled(credential_path: str) -> bool:
    """Check if credits-bypass is disabled for this credential via env var.

    Per-credential override: CODEX_IGNORE_CREDITS_<SLUG>=true
    Global override:         CODEX_IGNORE_CREDITS=true

    When set, the credential is treated as if it has no paid credits,
    meaning included-window exhaustion will block routing (old behavior).
    """
    global_flag = os.environ.get("CODEX_IGNORE_CREDITS", "").lower()
    if global_flag in ("1", "true", "yes"):
        return True

    # Derive stable_id from credential path (email::account_id or filename)
    if credential_path.startswith("env://"):
        slug_source = credential_path.split("/")[-1]
    else:
        # For file paths, the proxy uses email::account_id as stable_id.
        # We can't resolve that here, so also check the filename stem.
        slug_source = Path(credential_path).stem

    slug = _slugify_stable_id(slug_source)
    per_cred = os.environ.get(f"CODEX_IGNORE_CREDITS_{slug}", "").lower()
    return per_cred in ("1", "true", "yes")
...
class CodexQuotaTracker:
        ...
        async def _push():
            try:
                ...
                if snapshot.primary:
                    ...
                    primary_window_name = primary_label.replace("-limit", "-window")
                    self._register_quota_group(primary_label, f"{provider_prefix}/_{primary_window_name}")
                    await self._usage_manager.update_quota_baseline(
                        accessor=credential_path,
                        model=f"{provider_prefix}/_{primary_window_name}",
                ...
                if snapshot.secondary:
                    ...
                    secondary_label = _window_label_from_seconds(
                        snapshot.secondary.window_minutes * 60 if snapshot.secondary.window_minutes else None
                    )
                    secondary_window_name = secondary_label.replace("-limit", "-window")
                    self._register_quota_group(secondary_label, f"{provider_prefix}/_{secondary_window_name}")
                    await self._usage_manager.update_quota_baseline(
                        accessor=credential_path,
                        model=f"{provider_prefix}/_{secondary_window_name}",
                        quota_max_requests=100,
                ...
                # Exception: if the credential has paid credits available,
                # included-window exhaustion should NOT block routing — OpenAI
                # will consume workspace credits instead.
                # Can be disabled per-credential via CODEX_IGNORE_CREDITS_<SLUG>.
                credits_available = (
                    snapshot.credits is not None
                    and (snapshot.credits.has_credits or snapshot.credits.unlimited)
                    and not _is_credits_bypass_disabled(credential_path)
                )
                ...
                for tier_key in reversed(QUOTA_TIER_HIERARCHY):
                    ...
                    if wd and wd["used_percent"] >= 100.0 and wd["reset_ts"] > now:
                        if credits_available:
                            lib_logger.debug(
                                f"Codex {tier_key} exhausted for "
                                f"{Path(credential_path).stem} but paid credits "
                                f"available — not blocking"
                            )
                        else:
                ...
                if not global_exhausted:
                    short_cred = (
                        credential_path.split("/")[-1]
                        if credential_path.startswith("env://")
                        else Path(credential_path).stem
                    )
                    for group in ["codex-global"] + list(window_tiers.keys()):
                        cleared = await self._usage_manager.clear_cooldown_if_exists(
                            credential_path,
                            model_or_group=group,
                        )
                        if cleared:
                            lib_logger.info(
                                f"Codex quota recovered for {short_cred} — "
                                f"{group} cooldown cleared"
                            )
            except Exception as e:
                lib_logger.debug(
    ...
    async def run_background_job(
        ...
        if not self._initial_baselines_fetched:
            try:
                ...
                for cred_path, data in quota_results.items():
                    ...
                    credits_data = data.get("credits")
                    has_paid_credits = (
                        credits_data is not None
                        and (credits_data.get("has_credits") or credits_data.get("unlimited"))
                        and not _is_credits_bypass_disabled(cred_path)
                    )
                    cred_id = _get_credential_identifier(cred_path)
                    if primary and primary.get("is_exhausted"):
                        if has_paid_credits:
                            credits_bypassed.append(f"{cred_id} (primary)")
                        else:
                            exhausted.append(f"{cred_id} (primary)")
                    if secondary and secondary.get("is_exhausted"):
                        if has_paid_credits:
                            credits_bypassed.append(f"{cred_id} (secondary)")
                        else:
                            exhausted.append(f"{cred_id} (secondary)")
                if exhausted:
                    lib_logger.warning(
                ...
                if credits_bypassed:
                    lib_logger.info(
                        f"Codex startup: {len(credits_bypassed)} window(s) exhausted "
                        f"but paid credits available (not blocking): "
                        f"{', '.join(credits_bypassed)}"
                    )
                if not exhausted and not credits_bypassed:
                    lib_logger.info(
                        f"Codex startup: {stored} baselines stored, no exhausted credentials"
                    )
            except Exception as e:
                lib_logger.error(f"Codex startup baseline fetch failed: {e}")
                # Will retry on next cycle since flag is still False
            return

        # Subsequent runs: refresh ALL credentials so we can detect recovery
        # for exhausted ones and keep baselines fresh for active ones.
        now = time.time()

        lib_logger.debug(
            f"Refreshing Codex quota for {len(credentials)} credentials"
        )
        ...
        success_count = 0
        cleared_count = 0
        for cred_path, result in zip(credentials, results):
            if isinstance(result, Exception) or not isinstance(result, CodexQuotaSnapshot):
                continue
            if result.status != "success":
                continue
            success_count += 1

            self._quota_cache[cred_path] = result

            window_tiers: Dict[str, Dict[str, Any]] = {}
            if result.primary:
                primary_label = _window_label_from_seconds(
                    result.primary.window_minutes * 60 if result.primary.window_minutes else None
                )
                window_tiers[primary_label] = {
                    "used_percent": result.primary.used_percent,
                    "reset_ts": result.primary.reset_at or 0,
                }
            if result.secondary:
                secondary_label = _window_label_from_seconds(
                    result.secondary.window_minutes * 60 if result.secondary.window_minutes else None
                )
                window_tiers[secondary_label] = {
                    "used_percent": result.secondary.used_percent,
                    "reset_ts": result.secondary.reset_at or 0,
                }

            # Check if paid credits bypass included-window exhaustion
            credits_available = (
                result.credits is not None
                and (result.credits.has_credits or result.credits.unlimited)
                and not _is_credits_bypass_disabled(cred_path)
            )
            global_exhausted = False
            for tier_key in reversed(QUOTA_TIER_HIERARCHY):
            ...
            if not global_exhausted:
                ...
                for group in ["codex-global"] + list(window_tiers.keys()):
                    ...
                    if cleared:
                        cleared_count += 1
                        lib_logger.info(
                            f"Codex background refresh: quota recovered for {short_cred} — "
                            f"{group} cooldown cleared"
                        )

        lib_logger.debug(
            f"Codex quota refresh complete: {success_count}/{len(credentials)} successful"
            + (f", {cleared_count} cooldown(s) cleared" if cleared_count else "")
        )

    # =========================================================================
    # DYNAMIC QUOTA GROUP REGISTRATION
    # =========================================================================

    def _register_quota_group(self, group_name: str, virtual_model: str) -> None:
        """Register a dynamic quota group with the provider's model_quota_groups.

        This ensures the quota viewer can discover and display the group.
        """
        if not hasattr(self, "model_quota_groups"):
            return

        if group_name not in self.model_quota_groups:
            self.model_quota_groups[group_name] = [virtual_model]
            lib_logger.debug(
                f"[Codex] Registered dynamic quota group: {group_name} -> {virtual_model}"
            )
    ...
    async def _store_baselines_to_usage_manager(
        ...
        for cred_path, quota_data in quota_results.items():
            ...
            # Exception: if the credential has paid credits available,
            # included-window exhaustion should NOT block routing.
            ...
            for tier_key in reversed(QUOTA_TIER_HIERARCHY):
                ...
                if wd and wd["used_percent"] >= 100.0 and wd["reset_ts"] > now:
                    if credits_available:
                        lib_logger.info(
                            f"Codex startup: {tier_key} exhausted for {short_cred} "
                            f"but paid credits available — not blocking"
                        )
                    else:
                        global_exhausted = True
            ...
            try:
                ...
                if not global_exhausted:
                    for group in ["codex-global"] + list(window_tiers.keys()):
                        ...
                        if cleared:
                            lib_logger.info(
                                f"Codex startup: stale {group} cooldown cleared "
                                f"for {short_cred} (API shows quota available)"
                            )
            except Exception as e:
                lib_logger.warning(
