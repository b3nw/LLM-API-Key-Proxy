    ...
    async def acompletion(
        ...
        **kwargs,
        ...
        if self.enable_request_logging:
            ...
            # Strip internal keys that aren't JSON serializable before logging
            # (_anthropic_payload is a Pydantic BaseModel logged separately by AnthropicHandler)
            loggable_kwargs = {
                k: v for k, v in kwargs.items()
                if not k.startswith("_")
            }
            transaction_logger.log_request(loggable_kwargs)
