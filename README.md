...
| **NanoGPT** | Routes Claude models natively to the Anthropic `/v1/messages` endpoint to preserve thinking and tool calling |
| **Codex** | Rewrites standard `v1/chat/completions` requests into the experimental `Responses API` format |
| **Gemma-3** | Converts `system` messages to `user` messages for API compatibility |
| **IFlow** | Strips `stream_options` to prevent HTTP 406 errors |
