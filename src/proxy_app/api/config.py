...
async def update_config(update: ConfigUpdate):
    ...
    for key, value in update.changes.items():
        if key in ("PROXY_API_KEY",) and value is None:
            continue
        if value is None:
        ...
        for f in sorted(oauth_dir.iterdir()):
            if f.is_file() and f.suffix == ".json" and "_oauth_" in f.name:
                ...
                try:
                    ...
                    info["email"] = meta.get("email") or data.get("email")
...
async def add_api_key(req: AddApiKeyRequest):
    ...
    set_key(env_file, key_name, req.key)
...
async def delete_api_key(provider: str, key_name: str, request: Request):
    ...
    # Remove from running proxy's in-memory state
    try:
        client = request.app.state.rotating_client
...
async def delete_oauth_credential(provider: str, filename: str, request: Request):
    ...
    if not str(target.resolve()).startswith(str(oauth_dir.resolve())):
        raise HTTPException(status_code=403, detail="Access denied")

    target.unlink()

    # Remove credential from the running proxy's in-memory state
    removed_from_proxy = False
    try:
        client = request.app.state.rotating_client
        cred_path = str(target)
        provider_lower = provider.lower()
        if provider_lower in client.all_credentials:
...
async def add_custom_provider(req: AddCustomProviderRequest):
    ...
    set_key(env_file, f"{provider_upper}_API_BASE", req.base_url)
    set_key(env_file, f"{provider_upper}_API_KEY", req.api_key)
...
async def update_model_filters(provider: str, filters: ModelFilterUpdate):
    ...
    if filters.ignore:
        set_key(env_file, ignore_key, ",".join(filters.ignore))
    else:
        unset_key(env_file, ignore_key)

    if filters.whitelist:
        set_key(env_file, whitelist_key, ",".join(filters.whitelist))
    else:
        unset_key(env_file, whitelist_key)
