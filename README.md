...
| **Ollama Cloud** | Free/paid inference via `ollama.com` with dynamic model discovery from `/api/tags`. Session + weekly quota tracking via HTML scraping of the settings page using a `__Secure-session` cookie. Supports 429-based reactive rotation as fallback |
...
UMANS_QUOTA_LIMIT=200  # optional, code_pro soft request limit override
...
# Models discovered dynamically from /api/tags. Session + weekly quota
# tracking via HTML scraping of ollama.com/settings (no public quota API).
