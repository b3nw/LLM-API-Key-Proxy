...
class OllamaCloudProvider(OllamaCloudQuotaTracker, ProviderInterface):
    ...
    # Ollama Cloud uses session + weekly quota windows.
    # All real models consume from the "session" pool (the tighter constraint).
    # The "_session" and "_weekly" synthetic models store scraped baselines.
    model_quota_groups = {
        "session": ["_session", "_weekly"],
    }

    def get_model_quota_group(self, model: str) -> Optional[str]:
        """All models share the session quota pool (the binding constraint)."""
        return "session"
    ...
    def parse_quota_error(
        ...
        # Ollama 429 responses
        if "rate limit" in body_lower or "too many requests" in body_lower:
            retry_after = None
            hours_match = re.search(r"(\d+)\s*hour", body_lower)
            if hours_match:
                retry_after = int(hours_match.group(1)) * 3600
            return {
                "retry_after": retry_after,
                "reason": "session_quota_exhausted",
            }

        # Session/weekly limit messages
        if "session" in body_lower and "limit" in body_lower:
            return {"retry_after": None, "reason": "session_quota_exhausted"}
        if "weekly" in body_lower and "limit" in body_lower:
            return {"retry_after": None, "reason": "weekly_quota_exhausted"}
