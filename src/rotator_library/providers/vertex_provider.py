...
class VertexProvider(ProviderInterface):
    ...
    async def get_models(self, api_key: str, client: httpx.AsyncClient) -> List[str]:
        """
        Discover available models via the Generative Language API.

        The Vertex AI publisher models endpoint doesn't support API key auth,
        but the generativelanguage.googleapis.com/v1beta/models endpoint does.
        We filter to chat-capable Gemini models only.
        """
        models = []
        ...
        # Suffixes for non-chat model variants to exclude
        exclude_suffixes = (
            "-tts",
            "-tts-preview",
            "-image",
        ...
        # Substrings for specialized models to exclude
        exclude_contains = (
            "robotics",
            "computer-use",
        )
        ...
        try:
            # Parse credential — api_key may be in project:key format
            ...
            for model_info in data.get("models", []):
                ...
                if model_name.startswith(exclude_prefixes):
                    continue
                if model_name.endswith(exclude_suffixes):
                    continue
                if any(s in model_name for s in exclude_contains):
                    continue

                # Only include gemini and gemma-4 models
                if model_name.startswith("gemini-"):
                    models.append(f"vertex/{model_name}")
                elif model_name == "gemma-4-26b-a4b-it":
                    # This specific model requires -maas suffix on Vertex OpenAI endpoint
                    models.append(f"vertex/{model_name}-maas")

            if models:
                lib_logger.info(
                    f"Discovered {len(models)} chat models for Vertex provider"
                )

        except Exception as e:
            lib_logger.warning(
                f"Failed to discover models from Generative Language API: {e}. "
                f"Falling back to static model list."
            )

        # If discovery failed or returned nothing, provide common defaults
        if not models:
            default_models = [
                "gemini-2.5-pro",
                "gemini-2.5-flash",
            ...
            lib_logger.info(
                f"Using {len(models)} default models for Vertex provider"
            )
