...
class RequestExecutor:
                    ...
                    try:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                return self._strip_thought_tags_from_response(response)
    ...
    def _strip_thought_tags_from_response(self, response: Any) -> Any:
        """
        Strip <thought>...</thought> blocks from a non-streaming response.

        Mutates the response in-place when possible; falls back to returning
        a plain dict for immutable Pydantic models.
        """
        ...
        stripped = self._THOUGHT_PATTERN.sub("", content)
        if stripped == content:
            return response
        ...
        if (
            ...
            response_dict["choices"][0]["message"]["content"] = stripped
