...
class OpencodeProvider(OpencodeQuotaTracker, ProviderInterface):
    ...
    def _ensure_moonshot_reasoning(self, kwargs: Dict[str, Any], model: str) -> None:
        """
        Moonshot/Kimi models with thinking mode enabled require that every
        assistant message containing tool_calls includes a reasoning_content
        field.  Standard OpenAI clients omit this field on historical turns
        that were not generated by Kimi, causing a 400 error:

            "thinking is enabled but reasoning_content is missing
             in assistant tool call message at index N"

        This mirrors _ensure_deepseek_v4_reasoning but is scoped to Moonshot
        models and only pads assistant messages that actually carry tool_calls.
        """
        ...
        if patched:
            kwargs["messages"] = new_messages
            lib_logger.debug(
                f"opencode_go: padded reasoning_content on {patched} assistant "
                f"tool-call message(s) for {model}"
            )
