...
class VertexProvider(ProviderInterface):
    ...
    def _is_gemini_3(model: str) -> bool:
        bare = model.replace("vertex/", "").replace("google/", "")
        return bare.startswith("gemini-3") or bare.startswith("gemini-3.1")
    ...
    def _inject_thought_signatures(self, messages: list, model: str) -> list:
        if not self._is_gemini_3(model):
            return messages
    ...
    async def get_models(self, api_key: str, client: httpx.AsyncClient) -> List[str]:
        """
        Discover available models via the ModelRegistry and Generative Language API.
        """
        discovered_models = set()

        # Prefixes/suffixes for non-chat models to exclude
        exclude_prefixes = ("gemini-embedding", "imagen-", "lyria-", "veo-", "nano-banana", "aqa")
        exclude_suffixes = ("-tts", "-tts-preview", "-image", "-image-preview", "customtools")
        exclude_contains = ("robotics", "computer-use")

        # 1. Try discovery via internal ModelRegistry (models.dev / OpenRouter)
        # This works even with restricted keys as it uses the proxy's background catalog.
        try:
            from ..model_info_service import get_model_info_service
            registry = get_model_info_service()
            if registry:
                raw_models = registry.get_all_source_models()
                for mid in raw_models.keys():
                    # IDs look like "google/model" or "openrouter/google/model"
                    parts = mid.split("/")
                    if "google" not in [p.lower() for p in parts]:
                        continue
                    
                    model_name = parts[-1]
                    if ":" in model_name: # Skip variants like ":free"
                        model_name = model_name.split(":")[0]

                    # Skip non-chat models
                    if model_name.startswith(exclude_prefixes) or model_name.endswith(exclude_suffixes):
                        continue
                    if any(s in model_name for s in exclude_contains):
                        continue

                    if model_name.startswith("gemini-") or model_name.startswith("gemma-"):
                        discovered_models.add(model_name)
        except Exception as e:
            lib_logger.debug(f"Registry-based discovery failed for Vertex: {e}")

        # 2. Fallback: Try discovery via Generative Language API (if key allows)
        if not discovered_models:
            try:
                _, actual_key = self._parse_credential(api_key)
                response = await client.get(
                    "https://generativelanguage.googleapis.com/v1beta/models",
                    headers={"x-goog-api-key": actual_key},
                    timeout=10.0,
                )
                ...
                lib_logger.debug(f"Native discovery failed for Vertex: {e}")

        # Final conversion to provider-prefixed format
        models = []
        for m in discovered_models:
            if m == "gemma-4-26b-a4b-it":
                models.append(f"vertex/{m}-maas") # Special handling for Gemma on Vertex
            else:
                models.append(f"vertex/{m}")

        if models:
            lib_logger.info(f"Discovered {len(models)} chat models for Vertex provider")
            return models

        # 3. Last Resort: Use static defaults
        default_models = [
            "gemini-2.5-pro",
            "gemini-2.5-flash",
    ...
    async def acompletion(
        ...
        messages = self._inject_thought_signatures(messages, model)
        ...
        for param in [
            ...
            "response_format", "stream", "reasoning_effort",
        ]:
            if param in kwargs and kwargs[param] is not None:
                payload[param] = kwargs[param]

        # Handle thinking/reasoning params
        if "thinking" in kwargs:
            payload.setdefault("extra_body", {})
            payload["extra_body"]["google"] = payload["extra_body"].get("google", {})
            ...
            if isinstance(thinking, dict):
                ...
                if "budget_tokens" in thinking:
                    payload["extra_body"]["google"]["thinking_config"]["thinking_budget"] = thinking["budget_tokens"]
