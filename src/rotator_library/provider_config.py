...
class ProviderConfig:
    ...
    # Provider names used by this proxy that differ from LiteLLM's provider
    # prefix.  Keys = proxy provider name, values = litellm provider prefix.
    # When model="google/gemini-2.5-flash", litellm needs "gemini/gemini-2.5-flash".
    ...
    def convert_for_litellm(self, **kwargs) -> Dict[str, Any]:
        ...
        # Remap provider prefix when proxy uses a different name than litellm
        ...
        if not api_base:
            # Fall back to the original provider name if it was remapped
            # (e.g. if 'google' was remapped to 'gemini' but GOOGLE_API_BASE was configured)
