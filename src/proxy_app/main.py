...
with _console.status("[dim]Loading LiteLLM library...", spinner="dots"):
    ...
    _original_raise_on_model_repetition = CustomStreamWrapper.raise_on_model_repetition

    def _patched_raise_on_model_repetition(self):
        if len(self.chunks) < 2:
            return
        if not self.chunks[-1].choices:
            return
        if not self.chunks[-2].choices:
            return
        _original_raise_on_model_repetition(self)
...
async def list_models(
    ...
    if enriched and hasattr(request.app.state, "model_info_service"):
        ...
        if model_info_service.is_ready:
            ...
            # Apply upstream-authoritative context windows from providers
            # that fetch model metadata during discovery (e.g. Umans).
            # Upstream /v1/models is authoritative for context_length;
            # catalog data (modelsdev) is only used for pricing.
            for prov_name in ("umans",):
                try:
                    prov_inst = client._get_provider_instance(prov_name)
                    if prov_inst and hasattr(prov_inst, "get_model_context_overrides"):
                        ctx_overrides = prov_inst.get_model_context_overrides()
                        if ctx_overrides:
                            for entry in enriched_data:
                                ctx_win = ctx_overrides.get(entry.get("id", ""))
                                if ctx_win:
                                    entry["context_window"] = ctx_win
                                    entry["context_length"] = ctx_win
                                    entry["max_input_tokens"] = ctx_win
