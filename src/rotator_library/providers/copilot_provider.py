...
class CopilotProvider(CopilotAuthBase, CopilotQuotaTracker, ProviderInterface):
    ...
    def _build_responses_body(
        ...
        **kwargs: Any,
        ...
        Converts chat/completions format to responses format:
        - messages → input (same array format is accepted)
        - system messages → instructions (extracted separately)
        ...
        for msg in messages:
            if msg.get("role") == "system":
                ...
                if isinstance(content, str):
                    ...
                    instructions = str(content) if not instructions else f"{instructions}\n{content}"
            else:
