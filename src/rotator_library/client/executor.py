...
class RequestExecutor:
    ...
    # Models emit reasoning inside inline tags within the regular content
    # field.  For non-streaming responses we strip them with regex.
    # Supported: <thought>...</thought>, <think>...</think>, and the bare
    # </think> pattern (Kimi K2 via umans) where all text before the close
    # tag is reasoning.
    _THINKING_PATTERNS = [
        re.compile(r"<thought>(.*?)</thought>", re.DOTALL),
        re.compile(r"<think>(.*?)</think>", re.DOTALL),
    ]
    _BARE_THINK_CLOSE = re.compile(r"^(.*?)</think>", re.DOTALL)

    def _extract_thought_tags_from_response(self, response: Any) -> Any:
        """
        Extract reasoning blocks from a non-streaming response.

        Handles ``<thought>...</thought>``, ``<think>...</think>``, and the
        bare ``</think>`` pattern (no opening tag — all text before the close
        tag is reasoning, as used by Kimi K2 via umans).

        Reasoning content is moved to ``message.reasoning_content`` (OpenAI
        ...
        # Handle bare </think> (no opening tag) — Kimi K2 pattern where
        # all content before the close tag is reasoning.
        if not reasoning_parts:
            bare_match = self._BARE_THINK_CLOSE.match(stripped)
            if bare_match:
                reasoning_parts.append(bare_match.group(1))
                stripped = stripped[bare_match.end():]
