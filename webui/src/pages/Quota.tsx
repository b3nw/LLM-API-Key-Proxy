...
import { RefreshCw, ChevronDown, ChevronRight, ArrowLeft } from "lucide-react"
...
function QuotaSummaryBars({ quotaGroups }: { quotaGroups?: Record<string, QuotaGroup> }) {
  ...
  for (const [groupName, group] of Object.entries(quotaGroups)) {
    for (const [windowName, win] of Object.entries(group.windows)) {
      ...
      windows.push({ label, key: `${groupName}-${windowName}`, pct: win.remaining_pct, remaining: win.total_remaining, max: win.total_max })
    }
  }
                    ...
                    {Object.entries(cred.model_usage).map(([model, usage]) => {
                      ...
                      return (
                        <TableRow key={model}>
                          ...
                          <TableCell className="text-xs text-right">{u.request_count as number ?? 0}</TableCell>
                          <TableCell className="text-xs text-right">{formatCost(u.approx_cost as number ?? 0)}</TableCell>
                        </TableRow>
                      )
                    })}
                  </TableBody>
